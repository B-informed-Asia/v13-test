...
class TreasuryForecast(models.Model):
    ...
    periodic_saldo = fields.Float(string='Periodic saldo', compute='_compute_periodic_saldo',
    ...
    forecast_analysis = fields.Text(string='Treasury Analysis',
                                    compute_sudo=False, compute='_compute_periodic_saldo')
    ...
    def compute_forecast_lines(self):
        for item in self:
            ...
            item.forecast_template_id = False
    ...
    def compute_forecast_data(self):
        for item in self:
            ...
            # collect all account move in this period
            ...
            move_list.filtered(lambda l: not l.treasury_date).set_treasury_date()

            # or those with treasury date within this range
            move_list = aml_obj.search([
                ('treasury_planning', '!=', False),
                ('treasury_date', '>=', item.date_start),
                ('treasury_date', '<=', item.date_end),
                ('forecast_id', '=', False),
            ])
            move_list.update({'forecast_id': item.id})
    ...
    def compute_final_balance(self):
        for item in self:
            ...
            if item.previous_forecast_id.final_balance:
                initial_balance = item.previous_forecast_id.final_balance
            if item.force_initial_balance != 0.0:
                initial_balance = item.force_initial_balance

            final_balance = initial_balance + item.periodic_saldo
