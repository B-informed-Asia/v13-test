...
class BankBalanceComputation(models.TransientModel):
    ...
    include_draft_invoices = fields.Boolean(string='Incl. draft moves')
    ...
    def get_bank_fc_query(self, fc_journal_list, date_start, date_end,
        ...
        query = """
                ...
                SELECT CAST('FBK' AS text) AS type, absl.id AS ID, absl.date,
                    ...
                    as amount, absl.cf_forecast, journal_id, NULL as kind
    ...
    def get_acc_move_query(self, date_start, date_end, company_domain):
        query = """
            ...
            SELECT CAST('FPL' AS text) AS type, aml.id AS ID,
                ...
                NULL AS journal_id, am.type as kind
    ...
    def get_draft_inv_query(self, date_start, date_end, company_domain):
        query = """
            ...
            SELECT CAST('DFT' AS text) AS type, aml.id AS ID,
                aml.treasury_date AS date, rp.name AS name, aml.company_id,
                CASE WHEN am.type in ('out_invoice', 'in_refund' )  THEN
                    aml.amount_residual ELSE (
                    aml.amount_residual * -1) END AS amount,
                NULL AS cf_forecast, NULL AS journal_id, NULL as kind
            FROM account_move_line aml
            LEFT JOIN res_partner rp ON (aml.partner_id = rp.id)
            LEFT JOIN account_move am ON (aml.move_id = am.id)
            WHERE am.state IN ('draft')
                AND aml.treasury_date BETWEEN '{}' AND '{}'
                AND aml.treasury_planning AND aml.company_id in {}
        """.format(date_start, date_end, company_domain)
        return query
        ...
        main_query = """
            WITH global_forecast AS (
                SELECT CAST('BNK' AS text) AS type, absl.id AS ID, absl.date,
                    ...
                    amount, absl.cf_forecast, journal_id, NULL as kind
                FROM account_bank_statement_line absl
            ...
            GROUP BY ID, type, name, date, amount, journal_id, kind
            ORDER BY date, id
        ...
        if self.show_chart:
            ...
            if not bokeh or bokeh.state != 'installed':
                raise UserError(_(
                    ...
                    'to see the graph chart! \n You can find an this module '
                    'in this repo : https://github.com/OCA/web/tree/13.0 .'))
    ...
    def compute_balance_at_date(self, journal, date, currency_id):
        ...
        if statememt_list:
            ...
            balance_curr = line_currency._convert(
                balance, currency_id, statement.company_id, statement.initial_date)
...
class BankBalanceDefault(models.Model):
    ...
    journal_ids = fields.Many2many(
        comodel_name='account.journal',
        string='Journals', domain="[('type', 'in', ['bank', 'cash'])]",)
    detailed_balance = fields.Boolean(string='Detailed balance')
    include_account_moves = fields.Boolean(string='Incl. account moves')
    include_draft_invoices = fields.Boolean(string='Incl. draft moves')
    include_sale_orders = fields.Boolean(string='Incl. sale orders')
